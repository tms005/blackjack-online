...
STOLIK[MAX_TABLES]; // istnieja 3 stoliki

//struktura wiadomosci
typedef struct Buffer {
short ID; //identyfikator funkcji , patrz dalej dostepne klucze
short ID_USR; // nadawany przez serwer klucz dla ka¿dego po³¹czonego z serwerem u¿ytkownika
	...
	DWORD WINAPI iSilnikGry(LPVOID ctx)
{
	...
	iMode=	Translate(sbMssg); // tutaj decydujemy co bedzie sie dzialo ;d
	...
	case 0: // tutaj wysylamy wiadomosc do swojego stolika lub do wszystkich
			sbMssgCLEAR();
			cMessageForward(); // tutaj modyfikacja nieznaczna jedynie potrzebna do dalszego rozeslania (do zaimplementowania)
			for(int i=0;i<MAX_CLIENTS;i++) if( iTable == gracze[i].IT ) send(gracze[i].sock  , cMessage, 275, 0);
			break;
	...
	case 1:
		...
		sbMssg.ID_USR=generuj_nowe_id_gracza();
		fprintf(login, "%d %s\n",sbMssg.ID_USR, wybierzHaslo());
		fclose(login);
...
{
	for(int i=0;i<275;i++) x->cChat[i]=0;
	x->ID=0;
	x->ID_USR= 0;
