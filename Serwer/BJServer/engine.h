	...
	case 1:
		{
			...
			{
				nofile=1;
				fputs ("Plik login nie istnieje! Tworzê nowy plik...\n", stderr);
			...
			vWybierzHaslo(&sbMssg, haslo);
			if(!nofile)
			{																							//jesli plik wczesniej nie istnial
			sbMssg.ID_USR = 1;
												//z wiadomosci otrzymujemy haslo
			...
			while(feof(login) == 0)
			{
				...
				if( ! strcmp(user.cName , imie ) ) already_exist=1;																	// i porownujemy je z istniejacymi
				else tab.push_back(user);																										
			}
		...
		if(already_exist==0)
				...
				vAddTerminalChar(haslo, MAX_PASS_LEN);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	case 2:
		{	
			...
			{
				fputs ("\nPlik login nie istnieje! Nikt nie ma prawa sie logowaæ!\n", stderr);
...
void  vWybierzHaslo(Buffer* sbMssg, char* out)
{
	...
	start=i+1;
	i++;
	while(sbMssg->cChat[i] !=' '||sbMssg->cChat[i] !='\0')i++;
	end=i-1;
	int k=0;

	while(start!=end) pass[k++]=sbMssg->cChat[start++];
	while(k>=0) *(out+k)=*(pass+(k--));
}
